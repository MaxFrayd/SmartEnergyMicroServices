...
import {
    ...
    InventoryProduct,
    InventoryTag,
    InventoryVendor, PageResponse, Supplier, SupplierPagination,
    SupplierSearchDto
...
export class InventoryService
{
    ...
    private _supplierPagination: BehaviorSubject<SupplierPagination | null> = new BehaviorSubject(null);
    ...
    // --- In your Angular service (same class that has getProducts) ---
    getSuppliers(
        page: number = 0,
        size: number = 10,
        sort: string = 'name',
        ...
        const params = {
            // the BE expects pageNum/pageSize; we still pass sort/order/search for parity
            pageNum: page,
            pageSize: size,
            sort,       // harmless if BE ignores today; future-proof if you add it later
            order,
            search
        } as any;

        // Minimal DTO that your BE can consume right now.
        // Adjust key/operation to match what your SupplierSpecificationBuilder expects.
        const body: SupplierSearchDto = {
            dataOption: 'AND',
            searchCriteriaList: search
                ? [{ key: 'name', operation: 'LIKE', value: search }]
                : []
        };

        return this._httpClient
            .post<PageResponse<Supplier>>('http://13.50.169.126:8072/smartenergy/api/suppliers/search/byCriteria', body, { params })
            .pipe(
                // Normalize Spring Page -> your FE shape
                map((pageResp) => ({
                    pagination: {
                        page,
                        size,
                        total: pageResp.totalElements,
                        totalPages: pageResp.totalPages,
                        sort,
                        order,
                        search
                    } as SupplierPagination,
                    suppliers: pageResp.content
                })),
                tap(({ pagination, suppliers }) => {
                    this._supplierPagination.next(pagination);
                    this._suppliers.next(suppliers);   // create BehaviorSubject<Supplier[]> like _products
                })
            );
    }
